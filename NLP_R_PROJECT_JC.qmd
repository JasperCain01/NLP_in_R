...
Finally, I used the topicmodels package to pull out the top topic terms for each journalist

```{r}

f_word_removal <- function(byline){
  removal_patterns<-"(\\d+)|(\\sam\\s)|(\\spm\\s)|(gmt)\\s|(bst)\\s|(afternoon)|(evening)|(early evening)|(summary)|(the\\s)|(says\\s)|(will\\s)|(\\sits\\s)|(said)|(can\\s)|(was\\s)|(not\\s)|(have\\s)|(this\\s)|(their\\s)|(they\\s)|(been\\s)|(are\\s)|(that\\s)|(also\\s)|(had\\s)|(were\\s)|(she\\s)|(her\\s)|(\\sis\\s)|"
...
f_topic_model <- function(byline){
  byline_LDA <- LDA(f_byline_dtm("Rachel Hall"),
                k=3,
                control=list(seed=1234))
  terms(byline_LDA,10) %>%
    tibble() %>%
    mutate(byline_name=byline)
}


byline_topics <- list_rbind(map(unique(df_articles_reduced$fields.byline),f_topic_model)) 
