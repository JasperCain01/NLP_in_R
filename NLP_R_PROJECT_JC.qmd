...
library(topicmodels) # for identify common topics
library(tm) # for creating document term matrices
...
I also used the topicmodels package to pull out the top topic terms for each journalist, to give a move rounded idea of the priorities for each author.
This would be interesting to peruse further, particularly triangulating with the newspaper sections these journalists most commonly write for.

```{r}

f_word_removal <- function(byline){
  removal_patterns<-"(\\d+)|(\\sam\\s)|(\\spm\\s)|(gmt)\\s|(bst)\\s|(afternoon)|(evening)|(early evening)|(summary)|(the\\s)|(says\\s)|(will\\s)|(\\sits\\s)|(said)|(can\\s)|(was\\s)|(not\\s)|(have\\s)|(this\\s)|(their\\s)|(they\\s)|(been\\s)|(are\\s)|(that\\s)|(also\\s)|(had\\s)|(were\\s)|(she\\s)|(her\\s)|(\\sis\\s)|(\\shis\\s)|(\\shas\\s)|"
removal_patterns <- paste0(removal_patterns,str_flatten(tm::stopwords("en"),collapse = "\\s|\\s"))

return(df_articles_reduced %>%
  filter(fields.byline==byline) %>%
    mutate(
  fields.text.clean =
    str_replace_all(
      str_remove_all(
        str_to_lower(fields.text),
        "[[:punct:]]"),
      removal_patterns,
      replacement = " ") %>%
  str_replace_all("\\s+"," ")))
...
f_byline_corpus<- function(byline){
  Corpus(
    VectorSource(
      f_word_removal(byline)$fields.text.clean))
}

f_byline_dtm <- function(byline){
  DocumentTermMatrix(
    f_byline_corpus(byline))
}


f_topic_model <- function(byline){
  byline_LDA <- LDA(f_byline_dtm(byline),
                k=3,
...
```{r topics_1}
f_topic_model("Graeme Wearden")
```

### Andrew Sparrow

```{r topics_2}
f_topic_model("Andrew Sparrow")
...
```{r topics_3}
f_topic_model("Denis Campbell Health policy editor")
```

### Rachel Hall

```{r topics_4}
f_topic_model("Rachel Hall")
```

### Archie Bland

```{r topics_5}
f_topic_model("Archie Bland")
```

### Nadeem Badshah

```{r topics_6}
f_topic_model("Nadeem Badshah")
```

### Nicola Davis Science correspondent

```{r topics_7}
f_topic_model("Nicola Davis Science correspondent")
```

### Andrew Gregory Health editor

```{r topics_8}
f_topic_model("Andrew Gregory Health editor")
```

### Nimo Omer

```{r topics_9}
f_topic_model("Nimo Omer")
```

### Jamie Grierson

```{r topics_10}
f_topic_model("Jamie Grierson")
```

### Steven Morris

```{r topics_11}
f_topic_model("Steven Morris")
```

### John Crace

```{r topics_12}
f_topic_model("John Crace" )
```
